...
`docs/` is a committed symlink to `sciencediscovery/docs`, a shallow sparse checkout of the [product repository](https://github.com/openJiuwen-ai/sciencediscovery) (`feat/jiuwenswarm`). The website renders the product's own `docs/en`, `docs/zh` and `docs/images`; nothing is copied or pinned here, so edit documentation upstream only.

- Locally, `npm run build` clones the checkout when it is missing. Run `npm run fetch:docs` to fast-forward it to the latest `feat/jiuwenswarm`. The checkout is git-ignored.
